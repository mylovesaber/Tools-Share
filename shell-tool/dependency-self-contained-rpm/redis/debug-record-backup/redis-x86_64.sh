...
# x86 定制路径后，默认的链接器路径全是错的，需要全删掉
sed '/RTLDLIST=.*/c RTLDLIST="/opt/project/redis/deps/usr/bin/ld.so"' -i /opt/project/redis/deps/usr/bin/ldd
cd ../../
rm -rf glibc-2.37
#########################################################################################
# Zlib-1.3
...
cd zlib-1.3
CFLAGS="-I/opt/project/redis/deps/usr/include" \
LDFLAGS="-Wl,--rpath=/opt/project/redis/deps/usr/lib -Wl,--dynamic-linker=/opt/project/redis/deps/usr/bin/ld.so" \
./configure --prefix=/opt/project/redis/deps/usr
make -j$(nproc)
#make check
make install
rm -fv /opt/project/redis/deps/usr/lib/libz.a

# 对照组
make DESTDIR=/home/build/zlib install
rm -fv /home/build/zlib/opt/project/redis/deps/usr/lib/libz.a
...
make prefix=/opt/project/redis/deps/usr install
...
cd zstd-1.5.4
# 注意根据 Makefile 中的构建方式，-fPIC 必须放 CFLAGS 中，LDFLAGS 追加 -shared，否则编译一定报错，同时必须单独 -L 指定 lz4 路径否则 zstd 不会支持 lz4
make -j$(nproc) \
    prefix=/opt/project/redis/deps/usr \
    CFLAGS=" -fPIC -I/opt/project/redis/deps/usr/include" \
    LDFLAGS="-Wl,--rpath=/opt/project/redis/deps/usr/lib -Wl,--dynamic-linker=/opt/project/redis/deps/usr/bin/ld.so -shared -L/opt/project/redis/deps/usr/lib"
...
echo -e "/opt/project/redis/deps/usr/bin/ld.so" | tr -d '\n' > libcap/loader.txt
...
make -j$(nproc) \
prefix=/opt/project/redis/deps/usr lib=lib \
CFLAGS="-I/opt/project/redis/deps/usr/include -fPIC" \
LDFLAGS="-Wl,--rpath=/opt/project/redis/deps/usr/lib -Wl,--dynamic-linker=/opt/project/redis/deps/usr/bin/ld.so"
...
make prefix=/opt/project/redis/deps/usr lib=lib install
...
CFLAGS="-I/opt/project/redis/deps/usr/include" \
LDFLAGS="-Wl,--rpath=/opt/project/redis/deps/usr/lib -Wl,--dynamic-linker=/opt/project/redis/deps/usr/bin/ld.so" \
./config --prefix=/opt/project/redis/deps/usr \
    --openssldir=/opt/project/redis/deps/etc/ssl \
    --libdir=lib \
    shared \
...
cp -p build/{libsystemd.so,libsystemd.so.0,"${soName}"} /opt/project/redis/deps/usr/lib
mkdir -p /opt/project/redis/deps/usr/lib/systemd
cp -p src/systemd/*.h /opt/project/redis/deps/usr/lib/systemd
cd ..
rm -rf systemd-254
...
# 以上修复暂时发现 redis 7 系列存在这毛病，而 redis 6 系列不存在，但 redis 6 系列存在两个问题（内存分页问题7也有，但修复了）：
# - 官方默认引导 arm 用户使用 libc ，并给出各种理由说 arm 架构上 jemalloc 无法使用（默认配置下编译报错）
# - arm 架构 cpu 所用的各种系统内核中的内存分页存在兼容性问题，需要一个能兼容各种内存分页的配置

# redis 6 系列在 arm 架构上使用 jemalloc 方法：


# 注意，mac 的 arm64 架构 cpu 的内存分页是 16k，部分 aarch64 的 linux 内核是 64k，还有 4k
...
make -j$(nproc) \
    BUILD_TLS=yes \
    USE_WERROR=0 \
    OPENSSL_PREFIX=/opt/project/redis/deps/usr \
    MALLOC=jemalloc \
    USE_SYSTEMD=yes \
    CFLAGS="-I/opt/project/redis/deps/usr/include -fPIC" \
    LDFLAGS="-Wl,--rpath=/opt/project/redis/deps/usr/lib -Wl,--dynamic-linker=/opt/project/redis/deps/usr/bin/ld.so"
