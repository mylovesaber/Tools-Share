...
options=$1
execLog=/var/log/hosts-tool/exec-$(date +"%Y-%m-%d").log
...
function Usage(){
	...
	echo -e "\n选项:\n"
    echo "run                        立即更新hosts"
	echo "update                     默认从 GitLab 升级该工具"
	echo "updatefrom gitlab|github   需指定下载源才能升级该工具"
    echo "                           可选选项为 gitlab 或 github"
	echo "remove                     该选项将将此工具和生成的各种文件从系统中移除并还原 host 文件内容"
	echo "help                       显示帮助信息并退出"
}
...
function Remove(){
    _warning "开始卸载工具"
    # if [[ "${systemType}" =~ "MacOS"|"ROUTER" ]]; then
    if [ "${systemType}" = "MacOS" ]; then
        crontab -l | grep -v "hosts-tool" | crontab -
    elif [[ "${systemType}" =~ "unRAID" ]]; then
    ...
    rm -rf /etc/hosts_combine /etc/githubhosts.new /etc/hosts.default /etc/hosts.bak
    _success "卸载完成，hosts 文件已清除所有添加的 GitHub host 信息，拜拜~~"
}
