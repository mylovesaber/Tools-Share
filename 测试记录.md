...
```bash
# 部署同步测试，需要运行一次查看首次添加信息，一次加上 -y 确认添加，再来一次查看重复情况
...
bash <(cat multi-sync-backup.sh) --sync_source_path "/root/tsource/" --sync_dest_path "/root/tdest" --sync_source_alias "ax108" --sync_dest_alias "ax119" --days 8 -G "test" -T "dir" -D "aaaa-aa-aa" -N "sync108-119" -O "12" -o "108119" -l "test" -E "12" -L "ax108"
...
```bash
# 部署备份测试，需要运行一次查看首次添加信息，一次加上 -y 确认添加，再来一次查看重复情况
...
bash <(cat multi-sync-backup.sh) --backup_source_path "/root/tsource/" --backup_dest_path "/root/tdestbak" --backup_source_alias "ax108" --backup_dest_alias "ax119" --days 8 -g "test" -t "dir" -d "aaaa-aa-aa" -n "bak108-119" -O "12" -o "108119" -l "test" -E "12" -L "ax108"
