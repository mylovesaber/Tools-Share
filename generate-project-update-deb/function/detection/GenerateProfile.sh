...
# 桌面图标文件名，如果留空则代表和项目前端名一致，如果指定则为自定义的名称
# 一是在更新前端防止有的底包中的桌面图标文件名和前端名称不同导致更新时出现残留配置名称
# 二是每次有前后端更新(不包括 MySQL 单独更新)时，桌面图标文件中的链接一定会被更新，如果没有更新前端的话，此选项必须被指定
desktop-file-name=""

# 指定 @RequestMapping 注解中的内容，此内容将写入打开的网页链接中，不要添加内容开头的 /，误加也会被删掉
